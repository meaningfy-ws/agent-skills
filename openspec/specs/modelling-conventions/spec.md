...
Define the `modelling-conventions` skill: the shared, representation-agnostic modelling-craft layer in
`meaningfy-architecture` reused by the modelling skills the way `cosmic-python` is reused by the build
skills. Governs what the catalogue SHALL ship for generic naming discipline, modelling anti-patterns, and
editor-guiding guardrails — including the two load-bearing principles (attributes decoupled into reusable
first-class properties; everything identified by a stable URI, implicit by default) — and the rule that
representation-specific mechanics stay with the consuming skill.
## Requirements
